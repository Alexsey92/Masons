(() => {
    ...
    function tabs() {
        ...
        if (tabs.length > 0) {
            ...
            tabs.forEach(((tabsBlock, index) => {
                ...
                tabsBlock.addEventListener("mousemove", setTabsAction);
                initTabs(tabsBlock);
            }));
    ...
    const tabss = document.querySelectorAll(".menu__tabs");
    const tabsTitle = document.querySelectorAll(".tabs__title");
    if (detect.mobile()) document.addEventListener("click", documentActionsPhone); else document.addEventListener("mousemove", documentActionsPc);
    if (screenWidth < 767.98) for (let i = 0; i < tabsTitle.length; i++) tabsTitle[i].classList.remove("_tab-active");
    function documentActionsPhone(e) {
        const targetElement = e.target;
        const classBox = targetElement.parentNode;
        const classTabs = classBox.nextElementSibling;
        if (targetElement.classList.contains("menu__button")) {
            for (let i = 0; i < buttons.length; i++) if (targetElement !== buttons[i]) {
                buttons[i].classList.remove("_active-link");
                buttons[i].classList.remove("_active-button");
                tabss[i].classList.remove("tabs_active");
            }
            classTabs.classList.toggle("tabs_active");
            targetElement.classList.toggle("_active-link");
            targetElement.classList.toggle("_active-button");
        } else if (!targetElement.closest(".menu__item")) for (let i = 0; i < buttons.length; i++) {
            buttons[i].classList.remove("_active-link");
            buttons[i].classList.remove("_active-button");
            tabss[i].classList.remove("tabs_active");
        }
    }
    function documentActionsPc(e) {
        const targetElement = e.target;
        const classBox = targetElement.parentNode;
        const classTabs = classBox.nextElementSibling;
        if (targetElement.classList.contains("menu__button")) {
            ...
            classTabs.classList.add("tabs_active");
